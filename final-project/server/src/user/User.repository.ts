...
export class UserRepository {
  async create(user: Omit<User, "id">): Promise<User> {
    ...
    try {
      ...
      const [roleRows]: [RowDataPacket[], unknown] = await connection.execute(`SELECT id FROM roles WHERE name = ?`, [user.role]);

      let roleId: number;
      if (roleRows.length > 0) {
        roleId = roleRows[0].id;
      } else {
        const [roleResult]: [ResultSetHeader, unknown] = await connection.execute(`INSERT INTO roles (name) VALUES (?)`, [user.role]);
        roleId = roleResult.insertId;
      }

      await connection.execute(`INSERT INTO user_roles (user_id, role_id) VALUES (?, ?)`, [newUserId, roleId]);
